...
def read_SN_sendeckyj(DATA_DIRECTORY, file_toplot):
    method = 'Sendeckyj'
    filepath = os.path.join(DATA_DIRECTORY, method, file_toplot)
    sendeckyj = pd.read_fwf(filepath)
...
def sub_r_ratio_whitney(linlog, R_RATIO):
    sub_stress_whitney = []
...
def plot_sub_SN(sub_stress_linlog, sub_n_cycles_linlog, sub_stress_loglog, sub_n_cycles_loglog, sub_stress_sendeckyj, sub_n_cycles_sendeckyj, sub_stress_whitney, sub_n_cycles_whitney):
    ...
    plotSNlinlog = figure(title = 'S-N curves', plot_width=1200, plot_height=800,
                  x_axis_label = "N", y_axis_label = "Maximum Cyclic Stress", x_axis_type = "log")
    plotSNlinlog.add_tools(HoverTool(tooltips=[("N", "@n_cycles"), ("Maximum cyclic stress", "@stress_param")]))
    plotSNlinlog.line(x = 'n_cycles', y = 'stress_param', source = ColumnDataSource(data = sub_linlog_sn))
    plotSNlinlog.line(x = 'n_cycles', y = 'stress_param', source = ColumnDataSource(data = sub_loglog_sn))
    plotSNlinlog.line(x = 'n_cycles', y = 'stress_param', source = ColumnDataSource(data = sub_sendeckyj_sn))
    plotSNlinlog.line(x = 'n_cycles', y = 'stress_param', source = ColumnDataSource(data = sub_whitney_sn))
