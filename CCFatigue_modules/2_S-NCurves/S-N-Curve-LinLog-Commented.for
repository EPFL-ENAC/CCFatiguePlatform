      ...
      Program LinLog
         ...
         real A,B,P,Q,stress_ratio(100),reliability_level(100),stress_level(100,10000)
         real stress_parameter(100,10000),cycles_to_failure(100,10000),X(100,10000),Y(100,10000)
         ...
         do
            m=m+1
            read(10,*,iostat=ierr) stress_ratio(NOR),reliability_level(NOR),stress_level(NOR,m),stress_parameter(NOR,m),cycles_to_failure(NOR,m)
            if (ierr /= 0) then
               m=m-1
               exit
            end if
            if (m.GE.2) then
               if (PreSR.NE.stress_ratio(NOR)) then
                  OldR=PreSR
                  PreSR=stress_ratio(NOR)
                  stress_ratio(NOR)=OldR
                  NOR=NOR+1
                  NOD(NOR-1)=m-1
                  
                  stress_level(NOR,1)=stress_level(NOR-1,m)
                  stress_parameter(NOR,1)=stress_parameter(NOR-1,m)
                  cycles_to_failure(NOR,1)=cycles_to_failure(NOR-1,m)
                  m=1
                  go to 20
               end if
            end if
            ...
            do j=1,NOR				!Log(cycles_to_failure)=A+B*Sigma....Y=A+B*X
               do m=1,NOD(j)
                  X(j,m)=stress_parameter(j,m)
                  Y(j,m)=log10(cycles_to_failure(j,m))
               end do
            end do
            ...
            do f=1,NOR
               ...
               Ssigma=stress_parameter(f,1)
               SumN=cycles_to_failure(f,1)
               SumlogN=log10(cycles_to_failure(f,1))
               SigmaRL(f,1,1)=stress_parameter(f,1)
               CycleRL(f,1,1)=cycles_to_failure(f,1)
               
               checknumber=1
               level(f)=0
               kk=1
               do i=2,NOD(f)
                  if (stress_level(f,i).eq.stress_level(f,i-1)) then
                     Ssigma=Ssigma+stress_parameter(f,i)
                     SumN=SumN+cycles_to_failure(f,i)
                     SumlogN=SumlogN+log10(cycles_to_failure(f,i))
                     kk=kk+1
                     SigmaRL(f,level(f)+1,kk)=stress_parameter(f,i)
                     CycleRL(f,level(f)+1,kk)=cycles_to_failure(f,i)
                     ...
                     if (checknumber.GE.2) then
                        ...
                        do j=i-checknumber, i-1
                           write(30,*) averagesigma(f,level(f)),cycles_to_failure(f,j),averageN(f,level(f))
                        end do
                     end if
                     checknumber=1
                     Ssigma=stress_parameter(f,i)
                     SumN=cycles_to_failure(f,i)
                     SumlogN=log10(cycles_to_failure(f,i))
                     kk=1
                     SigmaRL(f,level(f)+1,kk)=stress_parameter(f,i)
                     CycleRL(f,level(f)+1,kk)=cycles_to_failure(f,i)
                  end if
               ...
               !for the last stress level in each stress_ratio-ratio
               level(f)=level(f)+1
               Replicate(f,level(f))=checknumber
               averagesigma(f,level(f))=Ssigma/checknumber
               averageN(f,level(f))=SumN/checknumber
               ...
               !do while (.NOT.EOF (10))
               !    m=m+1
               !	read(30,*) stress_parameter(f,m), cycles_to_failure(f,m)
               !end do
               ...
               !	checkmark=0
               !	do p=1,i-1,1
               !		if (stress_parameter(f,i).eq.stress_parameter(f,p)) then
               checkmark=1
               !		end if
               !	end do
               ...
               !		kk=1
               !		strlev=strlev+1
               !		S(strlev)=stress_parameter(f,i)
               !		if (cycles_to_failure(f,i).LT.rrout) then
               !			k=k+1
               !			noc(strlev,k)=cycles_to_failure(f,i)
               !		end if
               !		do j=i+1,m,1
               !			if ((stress_parameter(f,i)-stress_parameter(f,j)).EQ.0) then
               !				if (cycles_to_failure(f,j).LT.rrout) then
               !
               !					k=k+1
               !					noc(strlev,k)=cycles_to_failure(f,j)
               ...
               do i=1,NOD(f)
                  Yb=Yb+Y(f,i)
                  Xb=Xb+X(f,i)
                  Nb=Nb+cycles_to_failure(f,i) ! TODO: ask Tassos why not reinitialized beetween loops
                  LNb=LNb+log10(cycles_to_failure(f,i)) ! TODO: ask Tassos why not reinitialized beetween loops
               end do
               Xb=Xb/NOD(f)
               Yb=Yb/NOD(f)
               Nb=Nb/NOD(f)	!average of all cycles_to_failure
               LNb=LNb/NOD(f)	!average of all Log(cycles_to_failure)
               ...
               B=P/Q			!Log(cycles_to_failure)=A+B*Sigma....Y=A+B*X
               A=Yb-B*Xb
               ...
               !***************************************************************
               do i=1,level(f)
                  FN=FN+(Replicate(f,i)*((A+B*averagesigma(f,i))-averagelogN(f,i))**2)
               ...
               ! Calculating and writing the fitted S-cycles_to_failure curve based on the found parameters
               ! ##########################################################################
               WRITE(20,*) '0'				!Seperator
               WRITE(20,*) stress_ratio(f)
               WRITE(20,*) RSQL
               WRITE(20,*) A
               WRITE(20,*) B
               WRITE(20,*) LRSQ
               ...
               WRITE(20,*) RSQ				!stress_ratio-square
               ...
               do NN=1,1000,50
                  ...
                  WRITE(20,*) stress_ratio(f),NN,SPN,SPNu,SPNl
               end do
