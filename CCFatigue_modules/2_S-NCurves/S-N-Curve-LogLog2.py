...
samples["q"] = samples.apply(
    lambda x: (x.log10_stress_parameter - stress_ratios_df.loc[x.stress_ratio_id].xb)
    ** 2,
    axis=1,
)
stress_ratios_df["q"] = (
...
stress_ratios_df["level"] = (
    samples[["stress_ratio_id", "stress_level"]].groupby("stress_ratio_id").nunique()
)
...
for stress_ratio_id in samples["stress_ratio_id"].unique():
    ...
    stress_ratio_df = stress_ratios_df.loc[stress_ratio_id]
    ...
    stress_parameter["stress_parameter"] = stress_parameter.apply(
        lambda x: stress_at_failure(
            stress_ratio_df.stress_parameter_aa, stress_ratio_df.stress_parameter_bb, x
        )
    )
