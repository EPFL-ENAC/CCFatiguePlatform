...
20	end do
	...
	p1=0	!No. of R between 0 and 1 and equal to 0
	p2=0	!No. of R less than 0
	p3=0	!No. of R greater than 1
	p4=0
		...
		if (Rtest(i).GE.0) then
				...
				do m=1,NOD 
					...
					if (abs(R(1,p1)).GT.1) then
						asigma(1,p1,m)=(1-(1/R(1,p1)))
     &					*maxsigma(1,p1,m)/2
						msigma(1,p1,m)=-(1+(1/R(1,p1)))
     &					*maxsigma(1,p1,m)/2
					else 
	...
	! Between Part 1 and 2  ########################################################
			if (p1.GE.1) then
				if (p2.GE.1) then
					if (Rp.LT.R(1,p1)) then 
						if (Rp.GE.R(2,1)) then !1111
